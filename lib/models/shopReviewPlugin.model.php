 ...
 */
class shopReviewPluginModel extends waNestedSetModel {
    ...
    protected $table = 'shop_review_plugin';

    public function getFullTree($domain, $offset = 0, $count = null, $order = null, array $options = array()) {
        // first lever reviews - reviews to products
        $reviews = $this->getReviews($domain, $offset, $count, $order, $options);
        if (!empty($reviews)) {

            $is_frontend = wa()->getEnv() == 'frontend';

            // extract reviews to reviews (comments)
            $sql = "SELECT * FROM {$this->table} WHERE domain = '" . $this->escape($domain) . "'
     ...
     * @return array
     */
    public function getReviews($domain, $offset = 0, $count = null, $order = null, array $options = array()) {
        if (!$domain) {
            return array();
        }
        $sql = "SELECT * FROM {$this->table} WHERE domain = '" . $this->escape($domain) . "' AND review_id = 0";
    ...
    public function getList($fields = '*,is_new,contact,domain', $options = array()) {
    ...
    private function workupList(&$data, $fields, $escape) {
            ...
            if ($field == 'contact') {
                ...
                foreach ($data as &$item) {
                    ...
                    $item['author'] = array_merge(
                            $author, isset($contacts[$item['contact_id']]) ? $contacts[$item['contact_id']] : array()
                    );
                    if ($escape) {
       ...
       /*     if ($field == 'product') {
                ...
                $products = $product_model->getByField('id', $domains, 'id');
                $image_size = wa()->getConfig()->getImageSize('crop_small');
                foreach ($data as &$item) {
                    if (isset($products[$item['domain']])) {
                        $product = $products[$item['domain']];
    ...
    public function count($domain = null, $reviews_only = true) {
        if (wa()->getEnv() == 'frontend') {
            return $this->countInFrontend($domain, $reviews_only);
        }
        ...
        $where = array();
        if ($domain) {
            $where[] = "domain = '" . $this->escape($domain) . "'";
        }
        if ($reviews_only) {
            $where[] = "review_id = 0";
        }
    ...
    private function countInFrontend($domain = null, $reviews_only = true) {
        if ($domain) {
            $where = "domain = '" . $this->escape($domain) . "'";
        } else {
            $where = "";
    ...
    /*
      private function recalcProductRating($domain, $rate, $inc = true) {
      if ($rate <= 0) {
      return;
      }
      $product_model = new shopProductModel();
      $product = $product_model->getById($domain);
      if ($inc) {
      $update = array(
      ...
      $product_model->updateById($domain, $update);
      }
     */
    ...
    public function changeStatus($review_id, $status) {
        ...
        if ($status == self::STATUS_DELETED) {
            //$this->recalcProductRating($review['domain'], $review['rate'], false);
        } else {
            //$this->recalcProductRating($review['domain'], $review['rate']);
        }
    ...
    /*
      public function deleteByProducts(array $domains) {
      $this->deleteByField('domain', $domains);
      }
     */
